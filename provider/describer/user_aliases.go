...
func ListUserAliases(ctx context.Context, handler *GoogleWorkspaceAPIHandler, stream *models.StreamSender) ([]models.Resource, error) {
	...
	go func() {
		...
		if err := processAliases(ctx, handler, GoogleWorkspaceChan, &wg); err != nil {
...
func processAliases(ctx context.Context, handler *GoogleWorkspaceAPIHandler, openaiChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var aliasesResp *admin.Aliases

	req := handler.Service.Users.Aliases.List(handler.CustomerID)
	...
	for _, alias := range aliasesResp.Aliases {
		...
		} else if aliasValuePtr, ok := alias.(*admin.UserAlias); ok {
			go func(alias *admin.UserAlias) {
				defer wg.Done()
				value := models.Resource{
					ID:   alias.Id,
