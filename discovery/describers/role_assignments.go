...
func ListRoleAssignments(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, stream *models.StreamSender) ([]models.Resource, error) {
...
func GetRoleAssignment(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*models.Resource, error) {
	...
	value := models.Resource{
		...
		Description: provider.RoleAssignmentDescription{
...
func processRoleAssignments(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, GoogleWorkspaceChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var roleAssignments []*admin.RoleAssignment
	var roleAssignmentsResp *admin.RoleAssignments
	pageToken := ""

	for {
		req := handler.AdminService.RoleAssignments.List(handler.CustomerID).MaxResults(provider.MaxPageResultsRoleAssignments)
		if pageToken != "" {
...
func processRoleAssignment(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*admin.RoleAssignment, error) {
