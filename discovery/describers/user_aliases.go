...
func ListUserAliases(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, stream *models.StreamSender) ([]models.Resource, error) {
	var wg sync.WaitGroup
	GoogleWorkspaceChan := make(chan models.Resource)
	errorChan := make(chan error, 1) // Buffered channel to capture errors

	users, err := provider.GetUsers(ctx, handler)
	if err != nil {
		return nil, err
	}
...
func processUserAliases(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, userID string, GoogleWorkspaceChan chan<- models.Resource, wg *sync.WaitGroup) error {
		...
		if aliasValue, ok := alias.(map[string]interface{}); ok {
			go func(aliasValue map[string]interface{}) {
				...
				value := models.Resource{
					ID:   id,
					Name: aliasName,
					Description: provider.UserAliasDescription{
