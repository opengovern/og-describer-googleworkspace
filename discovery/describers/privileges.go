...
func ListPrivileges(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, stream *models.StreamSender) ([]models.Resource, error) {
...
func processPrivileges(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, GoogleWorkspaceChan chan<- models.Resource, wg *sync.WaitGroup) error {
	...
	for _, privilege := range privilegesResp.Items {
		...
		go func(privilege *admin.Privilege) {
			...
			value := models.Resource{
				...
				Description: provider.PrivilegeDescription{
					Privilege: *privilege,
				},
			}
