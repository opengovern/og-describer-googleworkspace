...
func ListUsers(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, stream *models.StreamSender) ([]models.Resource, error) {
...
func GetUser(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*models.Resource, error) {
...
func processUsers(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, GoogleWorkspaceChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var users []*admin.User
	var usersResp *admin.Users
	pageToken := ""

	for {
		req := handler.AdminService.Users.List().Customer(handler.CustomerID).MaxResults(provider.MaxPageResultsUsers)
		if pageToken != "" {
...
func processUser(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*admin.User, error) {
	var user *admin.User
	var status *int
