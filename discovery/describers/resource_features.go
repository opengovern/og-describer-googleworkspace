...
func ListResourceFeatures(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, stream *models.StreamSender) ([]models.Resource, error) {
...
func GetResourceFeature(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*models.Resource, error) {
	...
	value := models.Resource{
		ID:   feature.Name,
		Name: feature.Name,
		Description: provider.ResourceFeatureDescription{
			Feature: *feature,
		},
	}
	return &value, nil
...
func processResourceFeatures(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, GoogleWorkspaceChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var features []*admin.Feature
	var featuresResp *admin.Features
	pageToken := ""

	for {
		req := handler.AdminService.Resources.Features.List(handler.CustomerID).MaxResults(provider.MaxPageResultsFeatures)
		if pageToken != "" {
...
func processResourceFeature(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*admin.Feature, error) {
