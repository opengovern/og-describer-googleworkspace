...
func ListResourceCalenders(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, stream *models.StreamSender) ([]models.Resource, error) {
...
func GetResourceCalender(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*models.Resource, error) {
	...
	value := models.Resource{
		...
		Description: provider.ResourceCalenderDescription{
...
func processResourceCalenders(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, GoogleWorkspaceChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var calenders []*admin.CalendarResource
	var calendersResp *admin.CalendarResources
	pageToken := ""

	for {
		req := handler.AdminService.Resources.Calendars.List(handler.CustomerID).MaxResults(provider.MaxPageResultsCalenders)
		if pageToken != "" {
...
func processResourceCalender(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*admin.CalendarResource, error) {
