...
func ListMobileDevices(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, stream *models.StreamSender) ([]models.Resource, error) {
...
func GetMobileDevice(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*models.Resource, error) {
	...
	value := models.Resource{
		...
		Description: provider.MobileDeviceDescription{
...
func processMobileDevices(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, GoogleWorkspaceChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var mobileDevices []*admin.MobileDevice
	var mobileDevicesResp *admin.MobileDevices
	pageToken := ""

	for {
		req := handler.AdminService.Mobiledevices.List(handler.CustomerID).MaxResults(provider.MaxPageResultsMobileDevices)
		if pageToken != "" {
...
func processMobileDevice(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*admin.MobileDevice, error) {
