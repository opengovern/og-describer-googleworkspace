...
func ListResourceBuildings(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, stream *models.StreamSender) ([]models.Resource, error) {
...
func GetResourceBuilding(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*models.Resource, error) {
	...
	value := models.Resource{
		...
		Description: provider.ResourceBuildingDescription{
			Building: *building,
		},
	}
	return &value, nil
...
func processResourceBuildings(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, GoogleWorkspaceChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var buildings []*admin.Building
	var buildingsResp *admin.Buildings
	pageToken := ""

	for {
		req := handler.AdminService.Resources.Buildings.List(handler.CustomerID).MaxResults(provider.MaxPageResultsBuildings)
		if pageToken != "" {
...
func processResourceBuilding(ctx context.Context, handler *provider.GoogleWorkspaceAPIHandler, resourceID string) (*admin.Building, error) {
